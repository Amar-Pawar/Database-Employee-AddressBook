...
CREATE TABLE `addressbook_service`.`contact_addressbook` (
  ...
  CONSTRAINT `Type`
    ...
    #UC13
    #ensuring all UC working fine with new table structure
    select count(*) from contact_addressbook where type = 'Family';
    
     #showing addressbook sorted alphabeticaly by name
     select * from contact where city = 'Pen' order by FirstName;
     
     #getting the count in addressbook by  city name
     select count(*) from contact where city = 'Pen';